...
Do not open, merge, or approve pull requests. Do not invent passing tests that skip the real path. Do not put secrets in files or chat. Do not say done while Definition of Done still has unchecked items you can satisfy. Prefer editing an existing note over creating a duplicate. ADRs under `adrs/` are optional background — do not block on writing new ones unless the human asks.
